...
speedx = [5, 4, 6]
speedy = [6, 2, 3]
colors = ['IndianRed', 'LightSalmon', 'Orange', 'Moccasin', 'SlateBlue', 'CornflowerBlue', 'DarkCyan', 'Coral']
balls = []
poolx = []  'координаты на поле х'
pooly = []
poolr = []
sc_main = 0
sc = 0
...
def new_ball():
    ...
    poolr.clear()
    global cr, cr_b    
    global x, y, r     'глобальные переменые х,у, и  радиуса шарика'
    global ball         'глобальная переменная шарика'
    global gde_b00s        'сам босс'
    global cheto_budet     'кол-во поинтов '
    canv.delete(ALL)
...
def score():
    ...
    global count, lose_count, l_count     'считает dead-очки из за промахов которые могут привести к проигрышу'
    canv.delete(count)
    canv.delete(l_count)
    global sc      'просто очки'
...
def lose():
    ''' начинает выполнятся с определенного количества промахов, останавливает шары на экране, не дает появлятся новым
    выводит на экран счет и записывает его в текстовы документ'''
    global speedy, speedx, speedy_boos, speedx_boss, cr, cr_b, text
    if lose_count >= end_game:
        speedy = [0, 0, 0]
        speedx = [0, 0, 0]
        speedy_boos = 0
